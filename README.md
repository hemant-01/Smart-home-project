...
8) The secret_key is a pre-shared value between the AWS and the particular registered user of AWS. This is similar to how Microsoft provides it's OS with a pre-shared unique key for each device which installs legitimate Windows. The secret_key is sent by the **local_system** to the AWS while sending the parameters. The **secret_key** is verified to be correct by the AWS to **authenticate the user**. All of this happens over a secure HTTPS channel, thus providing guarantees when sending data over the channel(Internet).
